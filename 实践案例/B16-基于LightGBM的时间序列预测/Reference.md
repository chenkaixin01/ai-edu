...
用$t_i$表示第i轮迭代得到的回归树。那么，在进行完前![](https://latex.codecogs.com/svg.latex?\inline&space;k)轮迭代之后，GBDT对训练数据点![](https://latex.codecogs.com/svg.latex?\inline&space;(x,y))（其中x为特征，y为标签）的预测值为![](https://latex.codecogs.com/svg.latex?\inline&space;f_k(x)=\sum_{i=1}^k&space;t_i(x))。则第![](https://latex.codecogs.com/svg.latex?\inline&space;k&plus;1)轮迭代训练使用的数据标签为
![](https://latex.codecogs.com/svg.latex?\inline&space;-\frac{\partial&space;l(y,&space;\hat{y})}{\partial&space;\hat{y}}|_{\hat{y}&space;=&space;t_k(x_i)})。
其中![](https://latex.codecogs.com/svg.latex?\inline&space;l(y,\hat{y}))表示单个样本点对损失函数的贡献。例如，当损失函数为均方误差时，![](https://latex.codecogs.com/svg.latex?\inline&space;l(y,&space;\hat{y})&space;=&space;\frac{1}{2}(y&space;-&space;\hat{y})&space;^&space;2)。因此，在第![](https://latex.codecogs.com/svg.latex?\inline&space;k&plus;1)轮迭代训练的决策树，实际上是拟合当前GBDT预测值对损失函数的负梯度。如果损失函数为均方误差，则负梯度为![](https://latex.codecogs.com/svg.latex?\inline&space;f_k(x)-y)，这实际上就是当前预测值与标签![](https://latex.codecogs.com/svg.latex?\inline&space;y)的残差。
...
3. 对回归树叶子的预测值进行L2正则。具体来说，在训练第![](https://latex.codecogs.com/svg.latex?\inline&space;k&plus;1)棵决策树时，在损失函数当中加入一项![](https://latex.codecogs.com/svg.latex?\inline&space;\beta&space;\sum_{l&space;\in&space;t_{k&plus;1}}&space;w_l^2)，其中![](https://latex.codecogs.com/svg.latex?\inline&space;l\in&space;t_{k&plus;1})表示决策树![](https://latex.codecogs.com/svg.latex?\inline&space;t_{k&plus;1})中的所有叶子，![](https://latex.codecogs.com/svg.latex?\inline&space;w_l)表示叶子![](https://latex.codecogs.com/svg.latex?\inline&space;l)上的预测值。
